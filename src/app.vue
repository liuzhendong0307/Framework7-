    ...
    <f7-view main>
      <f7-page>
        ...
        <div>
          ...
          <input type="button" value="投票" @click="$store.commit('ADDNODEVOTE')">票数:{{$store.state.nodeVoteCount}}
        </div>
        <hr/>
        <div>
          ...
          <input type="button" value="投票" @click="$store.commit('ADDVUEVOTE')">票数：{{$store.state.vueVoteCount}}
        </div>
        <div>
          <input type="button" value="加2" @click="$store.commit('ADDNODEVOTE',2)">票数:{{$store.state.nodeVoteCount}}
        </div>
      </f7-page>
    </f7-view>
...
export default {
  ...
  created() {
    ...
    window.onload = function(){
      console.log(that.$$('.block')[0].innerHTML)
    }
//    this.app.views
  },
  methods: {
